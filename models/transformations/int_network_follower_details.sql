with
    ...
    internal_followers as (
        ...
        join users b on a.follower_id = b.user_id
        group by a.user_id
    ),
    internal_following as (
        select a.follower_id as user_id, list(b.user_name) as following
        from {{ ref("stg_followers") }} a
    ...
    combined as (
        ...
        from user_stats a
        left join internal_followers b on a.user_id = b.user_id
        left join internal_following c on a.user_id = c.user_id
    )
select
    *,
    case
