with
	...
	issue_metrics as (
		select
			...
			metric_timestamp,
			user_id,
			SUM(metric_count) OVER(partition by repo_id ORDER BY metric_timestamp
     ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
          AS metric_count
		from (
			select * from issue_metrics_open union all select * from issue_metrics_closed
		)
	),
	pr_metrics_open as (
		select
	...
	codebase_size_metrics as (
     ...
     ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
          AS metric_count
		from (
			select * from commit_metrics_added union all select * from commit_metrics_deleted
		)
	),
	commit_metrics as (
		select
			...
			metric_timestamp,
			user_id,
			count(metric_id) OVER(partition by repo_id ORDER BY metric_timestamp
     ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
          AS metric_count
		from (
			select * from commit_metrics_added union all select * from commit_metrics_deleted
